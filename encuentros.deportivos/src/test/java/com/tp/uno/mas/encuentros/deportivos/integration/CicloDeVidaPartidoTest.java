...
class CicloDeVidaPartidoTest {
    ...
    void setUp() {
        ...
        partido = partidoController.crearPartido(factory, "2024-12-20 15:00", ubicacion, organizador);
    }
    ...
    void testCancelacionEnDiferentesEstados() {
        ...
        TenisFactory factory = new TenisFactory();
        Ubicacion ubicacion = new Ubicacion(-34.6037f, -58.3816f, 5.0f);
        Partido partido2 = partidoController.crearPartido(factory, "2024-12-21 15:00", ubicacion, organizador);
        ...
        assertTrue(partidoController.cancelarPartido(partido2));
        
        // Crear tercer partido para probar Confirmado
        Partido partido3 = partidoController.crearPartido(factory, "2024-12-22 15:00", ubicacion, organizador);
        partido3.cambiarEstado(new Confirmado());
        assertTrue(partido3.getEstadoActual().puedeCancelar());
        assertTrue(partidoController.cancelarPartido(partido3));
        
        // En EnJuego no se puede cancelar
        Partido partido4 = partidoController.crearPartido(factory, "2024-12-23 15:00", ubicacion, organizador);
        partido4.cambiarEstado(new EnJuego());
        assertFalse(partido4.getEstadoActual().puedeCancelar());
        assertFalse(partidoController.cancelarPartido(partido4));
    }
    ...
    void testEstadosCancelableYFinalNoPermitenOperaciones() {
        ...
        assertFalse(partidoController.agregarJugador(partido, jugador1));
        assertFalse(partidoController.confirmarPartido(partido));
        assertFalse(partidoController.iniciarPartido(partido));
        assertFalse(partidoController.finalizarPartido(partido));
        assertFalse(partidoController.cancelarPartido(partido)); // Ya está cancelado
        ...
        assertFalse(partidoController.confirmarPartido(partido));
        assertFalse(partidoController.iniciarPartido(partido));
        assertFalse(partidoController.finalizarPartido(partido)); // Ya está finalizado
        assertFalse(partidoController.cancelarPartido(partido));
    }
    ...
    void testTransicionAutomaticaAlCompletarPartido() {
        ...
        boolean agregado = partidoController.agregarJugador(partido, jugador1);
